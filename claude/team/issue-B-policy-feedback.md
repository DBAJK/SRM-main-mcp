...
작성 2026-09-23 · C(에이전트) · 대상 B(②③⑤ 소유)

에이전트를 실제 MCP 서버 5개에 붙여 돌린 결과 세 가지가 드러났다. 셋 다
에이전트 쪽에서는 우회만 가능하고 근본 해결은 ② 또는 ⑤에 있다.
...
LLM 판단자로 돌린 60스텝 `mixed` 실행에서는 아예 선택조차 되지 않았다
(`rule_based` 45회 / `lstm_forecast` 0회, 45스텝 시점).
...
```
12스텝 실행   SLA 위반 11/12
45스텝 실행   SLA 위반 37/45  (82%)
```
...
다만 지금은 반대쪽 대가가 크다. SLA 위반 82% 는 `rule_based` 를 사실상
쓸 수 없는 정책으로 만들고, 그 결과 신뢰도가 붕괴해 개입률이 76% 까지
올라간다. **"개입은 줄었는데 SLA 위반은 늘지 않았다"** 를 보이려는 실험에서
두 지표가 동시에 최악이 된다.
...
이 셋이 함께 걸려 현재 상태는 이렇다.

```
LLM 이 상황을 다양하게 읽는다        normal 21 · special_event 19 · iot_surge 5
그런데 고를 정책이 하나뿐            rule_based 45 / 45
그 정책이 SLA 를 82% 놓친다
→ 신뢰도 붕괴 → 개입 76%
```

**판단은 살아 있으나 선택지가 없다.** 1번이 풀리지 않으면 "에이전트가 정책을
선택한다"는 논문의 서사를 실증할 수 없다.
