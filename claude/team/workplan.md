...
> **2026-09-26 이후의 순서는 [`workplan-2.md`](workplan-2.md) 가 정본이다.** B-1~B-4 · A-1 · A-2 가 들어온 뒤의
> 검증 · 남은 막힘 · 담당별 작업이 거기 있다. 이 문서의 진단(§1)과 M-0 기준값(§4b)은 그대로 유효하고,
> §5 의 B-3 판정은 workplan-2 §5 로 대체됐다. 아래 "셋이 어긋나면 이 문서가 최신" 은 1차 기준이다.
...
| 선택지 | (a) 상수 유지 (b) baseline 행동 (c1) 현재 관측 전문가 `a*(obs_t)` (c2) 완전 오라클 `a*(obs_{t+1})` |
| **권고** | **(c1).** "필요한 스텝만 골라 올린다"는 서사와 맞고, 개입 정밀도가 곧 기여가 된다. (c2)는 사람이 다음 스텝 트래픽까지 아는 가정이라 심사에서 공격받는다. ④ 코드라 A 소유 · 팀 결정. 상세는 §4b |
...
| (c1) 현재 관측 전문가 `a*(obs_t)` | 부른 스텝이 대체로 좋아짐 | "**필요한 스텝만 골라 부른다**" — 정밀도가 곧 기여 |
| (c2) 완전 오라클 `a*(obs_{t+1})` | 부른 스텝은 확실히 충족 | 같음. 다만 사람이 미래를 아는 가정 |

**오라클** = 정답을 완벽히 아는 가상의 존재. 현실 성능이 아니라 **"사람이 완벽할 때의 상한"** 을 재는 장치다.
`a*` 는 ⑤ 가 채점에 이미 쓰는 식(`feedback/scoring.py ideal_allocation` — `traffic ÷ (θ × capacity)` 를 합 1로)
이라 새로 만들 것이 없다.

**(c1) 을 권고한다.** 채점은 obs_{t+1} 로 하므로 (c2) 는 사람이 다음 스텝 트래픽까지 안다는 가정이 된다.
(c1) 은 지금 관측만 쓰되 정확히 계산하는 사람이라 미래 지식 없이 "개입하면 좋아진다"가 성립하고, 심사에서
공격받을 여지가 적다. 결과는 "현실 성능"이 아니라 "사람 모형 (c1) 하의 상한"으로 보고한다.

④ 코드(`book.py` `record_escalation` 의 `fallback`)를 바꾸는 일이라 A 소유이고 팀 결정이 필요하다.
어느 쪽이든 **논문에 사람 모형을 명시**해야 한다.
