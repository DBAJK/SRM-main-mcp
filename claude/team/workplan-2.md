...
| 심판 | 위반 0 | 위반 0 | `ignored_escalation` 2 — **심판 오탐**(C-19) · `record_after_apply` 1 — 9번째, 실제 순서 위반 |
...
| 규칙상 | 위반이 아니다. 판정은 고른 정책 기준이다. 다만 지금 심판은 기록 직전의 **마지막** 판정만 봐서, 미달 정책을 나중에 계산하면 `ignored_escalation` 으로 잘못 센다(C-19) |
...
**C-19 · 심판 `ignored_escalation` 오탐** — 의존 없음
