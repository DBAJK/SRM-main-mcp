...
|---|---|---|---|
| 상황 인지 arm1 − baseline | −0.144 (12 · 1 · 2) | −0.106 (11 · 2 · 2) | 정답을 아는 쪽이 여전히 더 나쁘다 — 목표표 오교정이 남았다 |
...
- 30스텝 · 위반 20개 안팎이라 AUC 는 ±0.1 흔들린다. 방향만 읽는다.

---

## 2. 결정 — 한 번의 회의에서

§4 의 M-1 · M-0b(무료, 약 80분)를 먼저 돌리면 아래 여섯 개를 숫자를 들고 한자리에서 정할 수 있다.
...
**M-1 결과(§1.5):** `combined` AUC normal 0.671 · emergency 0.488. 1차 기준을 그대로 대면 emergency 는 (b)
선행 신호 게이트, normal 은 회색 지대다. 두 시나리오 모두 `worst u/θ`(0.708 · 0.646)가 더 높아 (b)의 후보다.
