...
**위반 보정 (B-1 · D1)** — `rule_based` 는 목표표를 낸 뒤, 임계를 넘은 슬라이스에 `min(0.1, (uᵢ−θᵢ)×0.2)` 를 더하고 이용률이 가장 낮은 슬라이스에서 같은 양을 뺀다 (원본 `ml_orchestrator_demo.py:446~457`). 환경변수 `SLICE_RULE_CORRECTION` 으로 끄며, 기본은 `on`. **아래 예시는 `off` 기준이다** — `on` 이면 같은 입력에 `{0.250, 0.765, −0.015}` 가 나온다. 합은 보존되고, 음수·0.8 초과의 클립과 재정규화는 ①의 `apply_allocation()` 몫이다. 적용 설정은 `rationale` 끝의 `(보정 on|off)` 로 남는다.
